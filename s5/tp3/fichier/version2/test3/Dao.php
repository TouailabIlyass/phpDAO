...
public function AfficheBoot($table,$action=true,$list=NULL,$reserv=false,$moyen=false)
{
...
///////////////////////
  ...
  $str.="</td>";
 if ($moyen) {
      $tab=unserialize($v->getPrimaryKey());
     $str.="<td>".$this->calculeNote($tab['id'])."</td>";
     //$tab=unserialize($v->getPrimaryKey());
     //print_r($tab);
     //die();
  }
...
public function calculeNote($id)
{
  $stmt=$this->pdo->prepare("select note from note where etudiant_id = $id");
  $stmt->execute();
$note=0;
$n=$stmt->rowCount();
  while ($v=$stmt->fetch()) {
