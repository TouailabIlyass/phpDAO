...
if (isset($_GET['table'],$_GET['id'],$_GET['action'])) {
	...
	else if($_GET['action']=='reserver')
	{
                  ...
                  $dao->reserver([$user->getId(),$tab['n_c']]);
                  header('Location: aff.php?table=reserv');
	...
	if ($_GET['action']=='ajouter') {
		    ...
		    header('Location: Ajout'.ucfirst($_GET['table']).'.php?action=1');
		}
	if (isset($_POST['Send'])) {
	...
	header('Location: aff.php?table='.$_GET['table'].'&action=1');
